...
addCouponValidation = [
    ...
    .isNumeric().withMessage('النسبة غير صحيحة'),
    
    check('expire')
    .notEmpty().withMessage('تاريخ الانتهاء مطلوب')
    .custom((value) => {
   
      if (!moment(value, ['YYYY-MM-DD', 'YYYY-M-D', 'YYYY/MM/DD', 'YYYY/M/D'], true).isValid()) {

        return Promise.reject(new Error('تاريخ الانتهاء غير صحيح'));
      }

    
      const expireDate = moment(value, ['YYYY-MM-DD', 'YYYY-M-D', 'YYYY/MM/DD', 'YYYY/M/D'], true); 
...
[
    ...
    .custom(async (value) => {
        ...
        if(!coupon)
            {
                return  Promise.reject(new Error('كود الكوبون غير صحيح'));
            }
    }),
    validationMiddleware
]
